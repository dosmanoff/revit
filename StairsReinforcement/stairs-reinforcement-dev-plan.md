...
## Phase 5 — Ниши
✅ **Группировка floor-моделей по связности** — `Geometry/BoxCluster.cs` (connected-components по
bbox, union-find) + тесты; `FloorStairSource` делит выбранные перекрытия на отдельные лестницы
(одна сборка на связную группу). Раньше все выбранные перекрытия = одна лестница.

Отложено: винтовые/криволинейные и забежные ступени; «ножничные» (scissor) сборки; муфты/каплеры;
дедуп нахлёстов соседних компонентов; импорт/экспорт настроек; объединение кнопок плагинов в один
аддин; уточнение хостинга арматуры на нативных Stairs.
