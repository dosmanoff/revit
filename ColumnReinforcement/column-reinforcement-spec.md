...
**Сейсмика.** **Non-seismic по умолчанию (SDC A-C).** Confinement zones и 135° hooks — опциональные фичи Фазы 2/3, не часть MVP. Дефолтный hook = 90° standard tie hook per ACI 318 §25.3.2.
...
* Hook angles (ACI 318 §25.3.2): standard tie hook — **90° (default)** или 135°. Для seismic — 135° с шестикратным dia extension (Фаза 3 опция).
* Lap splice length (ACI 318 §25.5): функция f'c, fy, dia, top-bar factor — реализуется отдельным калькулятором (Фаза 2).
* Confinement length `lo` для SMRF (ACI 318 §18.7.5.1): `max(h_col, 1/6·clear_height, 18″)` — опциональная фича Фазы 2/3, не дефолт.
...
* Конфиг (JSON, Imperial defaults):
  ...
  ```
  Defaults подобраны под колонну 16″×16″ с #8 продольной и #4 хомутом — типовой non-seismic случай. Spacing 8″ — ACI §25.7.2 `min(16·1.0, 48·0.5, 16) = 16″`, берём с запасом 2x.
...
## 8. Решения и оставшиеся вопросы

### Зафиксированные решения (2026-05-26)

1. **Non-seismic (SDC A-C).** Дефолтный hook = 90° standard tie hook. Seismic (135° hooks + confinement) — опция Фазы 3.
2. **Ribbon tab = «SmartTools»** — общий для всех плагинов в репо (Wall/Slab/Column/будущие). Создание идемпотентно: если таб уже есть, добавляем только панель/кнопку.
3. **`RebarBarType` / `RebarHookType` lookup — strict.** Имя берётся из конфига, ищется по точному `.Name` в документе; если нет — per-column ошибка в `RunResult`, никакого auto-create. Имена типов задаются пользователем вручную.

Эти решения зафиксированы в memory как `repo-conventions` и применяются ко всем плагинам в этом репо.

### Оставшиеся вопросы (Фаза 2+)

1. **Группировка нужна сразу в Фазе 2?** Большинство US-практик — одно армирование на проект, или часто требуется деление по этажам/типам сечения? От ответа зависит, стоит ли поднимать Фазу 3 раньше Фазы 2.
2. **Калькулятор ℓd/lap-splice по ACI 318** — встроенный в Фазу 2 или достаточно вводить число руками (рассчитывая по таблицам ACI/CRSI отдельно)?
3. **`#14` и `#18`** — обычно для крупных колонн mat foundations; реально ли встречаются в ваших проектах? Это влияет на min/max ranges в UI dropdowns.
