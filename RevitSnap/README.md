...
| Параметр | Малая модель (5.9k элементов, 126 видов) | Реальная (144k элементов, 566 видов) |
|---|---|---|
| Снапшот | 22 с, 0.9 МБ (gz) | **163 с, 14 МБ (gz)** |
| Дифф пары снапшотов | < 1 с | 9 с, пик памяти 115 МБ |

Снапшот пишет в лог время по фазам. На реальной модели после оптимизации (было 350 с / 21 МБ на
схеме 1): экземпляры 84 с, виды 59 с, типы 19 с, остальное — доли секунды. Что дало ускорение:
словарь `paramDefs` вместо повторения метаданных в каждом параметре, отказ от `AsValueString` для
строковых параметров (58 с на модели), кэш типов/уровней/стадий/воркcетов и сбор листов двумя
общими коллекторами вместо двух на каждый лист — последнее одно снимало 96.6 с из 350.
