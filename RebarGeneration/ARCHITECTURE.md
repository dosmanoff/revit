...
## 10. Универсальный плагин: что реализовано

Фаза 3 сделана в виде отдельного плагина `RebarGeneration` (см. [`README.md`](README.md)). Ключевое решение — **универсальная оболочка, сменные генераторы**: унифицируется сантехника, а не предметная область.

### Почему не «один генератор на всё»

Транзакции, ключи, кэши, наборы, отчёт, журнал — одинаковы для плиты, стены, колонны и ростверка. Геометрия — нет: `SlabReinforcement` силён не транзакциями, а классификацией проёмов, смежностью границ и разбивкой на нахлёсты; у стен своя обрезка по скосу, у колонн — зоны раскрепления и выпуски. Универсальный генератор геометрии выродился бы либо в общий знаменатель (прямые стержни в коробке), либо в объединение трёх схем с гигантским `switch`.

Отсюда `IRebarGenerator`: генератор отвечает **только** за геометрию (группа задания + контекст хоста → список наборов в футах), всё остальное даёт раннер. Добавить тип хоста = написать генератор и зарегистрировать его.

Универсальный запасной путь — генератор `polyline` («вот кривые, вот шаг и количество»). Он закрывает всё, для чего предметного генератора нет, и это продуктизированная версия ручного скрипта из навыка: геометрию считает вызывающая сторона, а транзакции, наборы, ключи, идемпотентность и отчёт по-прежнему даёт плагин.

### Фасад на строках — и почему именно так

```csharp
public static string Run(Document doc, string jobJsonPath);   // JSON in → JSON out
```

Единственное непроверенное звено плана было маршалингом .NET-генериков через границу IronPython. Фасад «строка внутрь, строка наружу» убирает его полностью: вызов из `/exec` — две строки без единого генерика. Тот же фасад даёт три транспорта (`/exec`, будущая команда commandset, лента), поэтому третий мост между агентом и Revit строить не нужно.

### Транзакции

```
TransactionGroup            ← один шаг Revit Undo на весь прогон
  └── Transaction на ХОСТ   ← падение откатывает только его армирование
  Assimilate() | RollBack() ← RollBack для dry-run: отчёт настоящий, модель чистая
```

Критерий дробления — хост, а не количество; счётчик `maxSetsPerTransaction` вторичен. Отказ группы не рвёт транзакцию: созданные ею элементы удаляются, группа помечается `failed`, остальные продолжают.

### Что закрыто из таблицы швов §6

| Шов | Как закрыт |
| --- | --- |
| `find_by_key` квадратичен | `KeyIndex` — один проход, дальше словарь; формат ключа совместим с revit-context |
| Журнал O(N²) | `RunJournal` — append-only JSONL, одна строка на прогон |
| Фундаменты без движка | генератор `footing` (прямоугольная подошва по габариту) + `polyline` для остального |
| Опечатка в задании стоит дорого | `JobValidator` до транзакции, ошибки одним списком |

`SlabApi.RunFromBrief` остаётся невыполненным осознанно: движки Slab/Wall/Column уже работают и вызываются из `/exec` напрямую, обёртки для них — оптимизация удобства, а не необходимость.

### Замеры — гипотеза подтвердилась

Проверено на `7-15 Terrace View Avenue_R3` через `/exec`, режим `Api.DryRun`
(счётчик арматуры до и после совпал):

| Путь | На элемент | На 1000 стержней |
| --- | --- | --- |
| Поштучный `create_rebar` | 3450 мс | ~57 мин |
...
**135× на худшем случае** — когда наборы неприменимы. Целевые 50–100× достигнуты
даже без главного рычага; с наборами разрыв на два порядка больше.

Заодно подтвердились три проектных решения: предупреждения ушли в отчёт вместо
модального диалога (16 штук, прогон не встал), `KeyIndex` нашёл в модели реальную
неоднозначность ключа `colcage`, обе группы на одном хосте легли в одну транзакцию.

### Найденное на живом сеансе — важно для эксплуатации

**Не грузить сборку с `G:\` через `clr.AddReferenceToFileAndPath`.** Попытка подгрузить свежесобранную DLL прямо из репозитория на Google-Drive-диске заблокировала обработчик pyRevit Routes: Revit перестал отвечать мосту. Правильный путь — установка в `%APPDATA%` через `deploy/install.ps1`, после чего тип доступен по имени, как у остальных плагинов.
